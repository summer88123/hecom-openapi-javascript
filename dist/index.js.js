class HecomError extends Error {
  constructor(message) {
    super(message);
    this.name = "HecomError";
  }
}
class NetHecomError extends HecomError {
...
class BizHecomError extends HecomError {
...
class AuthService {
  ...
  async getAccessToken() {
    ...
    this.expiresIn = Date.now() + response.data.expires_in * 1e3;
...
class ObjectService {
   ...
   */
  async getObjectDescription(metaName) {
    return this.request("GET", `/oapi/v1/data/objects/${metaName}/description`);
  }
}
function buildQueryUrl(metaName, { selectFields, pageNo, pageSize, query }, basePath = "/v1/data/objects", defaultPageSize = 10) {
  const queryStr = query ? Object.keys(query).reduce((pre, cur) => {
    return `&${pre}${cur}=${query[cur]}`;
  }, "") : "";
  const selectFieldsStr = Array.isArray(selectFields) && selectFields.length > 0 ? selectFields.join(",") : "code,name";
...
class DataService {
   ...
   * @returns 创建的业务数据code
   */
  async createData(metaName, data) {
   ...
   */
  async updateData(metaName, code, data) {
   ...
   */
  async batchCreateData(metaName, records) {
   ...
   * @returns 删除的业务数据code
   */
  async deleteData(metaName, code) {
   ...
   */
  async batchUpdateData(metaName, records) {
    if (records.length > 30) {
   ...
   * @returns 业务数据列表
   */
  async queryData(metaName, options) {
   ...
   * @returns 业务数据列表
   */
  async queryAuxiliaryData(metaName, options) {
   ...
   */
  async transferOwner(metaName, code, newOwner, addTeam, deptFollowNewOwner) {
    if (!code) {
...
class UserService {
   ...
   * @returns 用户code
   */
  async updateUser(code, userData) {
   ...
   * @returns 用户code
   */
  async resumeUser(code, dept, phone) {
   ...
   * @returns 用户code
   */
  async transferUserDept(code, oldDeptCode, newDeptCode, transferData) {
...
class DeptService {
   ...
   * @returns 组织code
   */
  async updateDept(code, deptData) {
   ...
   * @returns 部门code
   */
  async disableDept(code, moveToDeptCode) {
   ...
   */
  async changeDeptParent(code, dept) {
...
class ConstantGroupService {
   ...
   */
  async getConstantOptions(groupName) {
   ...
   * @returns ConstantOption
   */
  async createConstantOption(groupName, name, label, parentName) {
   ...
   * @returns ConstantOption
   */
  async updateConstantOption(groupName, optionName, label) {
